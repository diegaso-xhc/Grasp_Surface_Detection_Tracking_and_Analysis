...
%%%% Call dependencies
dependencies;
load('MT_RH_995W_1400 015.mat') % Load the trial info we are trying to evaluate
cs_st = st;
...
ob = 2; % Object we have selected for the experiment
...
get_frames_opt(n_frames, mks, n_mks, cs_st, n_hand, 'test_data'); % Creates the required frame file to be used by the python optimization algorithm
system(strcat('python3.7 ./Fit_MANO/opt_MANO_MoCap.py --sequencedata', ' ./Trials/', 'test_data.mat')); % Calls an optimization algorithm in Python which uses the MANO hand model and runs the previously 
                                                                                                        % recorded marker sequence
betas = rand(1,10)*-6;
fr_in = 990;
fr_end = 1030;
[betas_str, fr_in_str, fr_end_str] = get_variable_str(betas, fr_in, fr_end);
system(strcat('python3.7 ./Fit_MANO/opt_MANO_MoCap.py --sequencedata', ' ./Trials/', 'test_data.mat',...
                                                    ' --betas', " ", betas_str,...
                                                    ' --fr_in', " ", fr_in_str,...
                                                    ' --fr_end', " ", fr_end_str))

load('verts_opt_test.mat')
load('faces_opt_test.mat')
vert_py = order_imported_matrix_n_row_cols(output_vertices);
faces_py = order_imported_matrix_n_row_cols(output_faces);
th = 10; % Threshold for the distance between meshes which we have set to detect the contact surfaces
yf_cell = calculate_contacts(th, Y, ob, y_track(fr_in: fr_end), vert_py, faces_py);
h = plot_results(vert_py, y_track(fr_in: fr_end), yf_cell, 1);
disp('Final!')                                      
